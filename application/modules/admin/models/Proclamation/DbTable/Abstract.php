 ...
 * @version $Id$
 *
 * @method Application_Model_Proclamation_Row createRow(array $data, string $defaultSource = null)
 * @method Application_Model_Proclamation_Rowset fetchAll(string|array|Zend_Db_Table_Select $where = null, string|array $order = null, int $count = null, int $offset = null)
 * @method Application_Model_Proclamation_Row fetchRow(string|array|Zend_Db_Table_Select $where = null, string|array $order = null, int $offset = null)
 * @method Application_Model_Proclamation_Rowset find()
 *
 */
abstract class Application_Model_Proclamation_DbTable_Abstract extends Zend_Db_Table_Abstract
{
    /**
     * @var string
     */
    protected $_name = 'proclamation';
  ...
  0 => 'Application_Model_Detailconsultproc_DbTable',
);
    ...
    protected $_referenceMap = array(        
        'idpersonnel' => array(
            'columns' => 'idpersonnel',
            'refTableClass' => 'Application_Model_Personnel_DbTable',
            'refColumns' => 'idpersonnel'
        )
    );
     ...
     * @var string
     */
    protected $_rowClass = 'Application_Model_Proclamation_Row';

    /**
     * @var string
     */
    protected $_rowsetClass = 'Application_Model_Proclamation_Rowset';
  ...
  'idpersonnel' => NULL,
    ...
    public function getDbSelectByParams($params = array(), $sortField = '', $sortOrder = '')
    {
        ...
        if (isset($params['codeproclm']) && !empty($params['codeproclm'])) {
            $select->where('codeproclm = ?', $params['codeproclm']);
        }

        if (isset($params['typeproclm']) && !empty($params['typeproclm'])) {
            $select->where('typeproclm = ?', $params['typeproclm']);
        }

        if (isset($params['decision']) && !empty($params['decision'])) {
            $select->where('decision = ?', $params['decision']);
        }
        ...
        if (isset($params['_kw']) && !empty($params['_kw'])) {
            ...
            if ('all' === $searchMode || 'typeproclm' === $searchMode) {
                $searchWheres[] = $dbAdapter->quoteInto('typeproclm LIKE ?', "%$keywords%");
            }

            if ('all' === $searchMode || 'decision' === $searchMode) {
                $searchWheres[] = $dbAdapter->quoteInto('decision LIKE ?', "%$keywords%");
            }
