 ...
 * @version $Id$
 *
 * @method Application_Model_AnneeUniversitaire_Row createRow(array $data, string $defaultSource = null)
 * @method Application_Model_AnneeUniversitaire_Rowset fetchAll(string|array|Zend_Db_Table_Select $where = null, string|array $order = null, int $count = null, int $offset = null)
 * @method Application_Model_AnneeUniversitaire_Row fetchRow(string|array|Zend_Db_Table_Select $where = null, string|array $order = null, int $offset = null)
 * @method Application_Model_AnneeUniversitaire_Rowset find()
 *
 */
abstract class Application_Model_AnneeUniversitaire_DbTable_Abstract extends Zend_Db_Table_Abstract
{
    /**
     * @var string
     */
    protected $_name = 'annee_universitaire';
  ...
  2 => 'Application_Model_Parcours_DbTable',
);
     ...
     * @var string
     */
    protected $_rowClass = 'Application_Model_AnneeUniversitaire_Row';

    /**
     * @var string
     */
    protected $_rowsetClass = 'Application_Model_AnneeUniversitaire_Rowset';
  ...
  'annee' => NULL,
    ...
    public function getDbSelectByParams($params = array(), $sortField = '', $sortOrder = '')
    {
        ...
        if (isset($params['idannee']) && !empty($params['idannee'])) {
            $select->where('idannee = ?', $params['idannee']);
        }

        if (isset($params['annee']) && !empty($params['annee'])) {
            $select->where('annee = ?', $params['annee']);
        }
        ...
        if (isset($params['_kw']) && !empty($params['_kw'])) {
            ...
            if ('all' === $searchMode || 'annee' === $searchMode) {
                $searchWheres[] = $dbAdapter->quoteInto('annee LIKE ?', "%$keywords%");
            }
